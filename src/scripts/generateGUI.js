...
function generateTaskList(){
    ...
    state.tasks
        ...
        const statusTag = createTag({
            ...
            tagAttrs: [{name: 'type', value: 'checkbox'}],
            tagId: `taskName-${id}`,
            tagEvents: [{type: 'change', cb: () => changeStatus(id, state)}]
        ...
        const clearBtn = createTag({
            tagName: 'button',
            tagText: 'Delete',
            tagAttrs: [{name: 'type', value: 'button'}],
            tagEvents:[{type:'click', cb: () => {deleteTask(id, state)}}]
